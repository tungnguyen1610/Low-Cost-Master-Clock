...
int clockadj_max_freq(clockid_t clkid)
{
	...
	memset(&tx, 0, sizeof(tx));
	if (clock_adjtime(clkid, &tx) < 0) 
		        fprintf(stderr, "failed to read out the clock frequency adjustment: %s\n", strerror(errno));
	else
		     ...
		     int64_t *offset, uint64_t *ts, int64_t *delay)
{
	...
	for (i = 0; i < readings; i++) {
		if (clock_gettime(sysclk, &tdst1) ||
			 ...
			 fprintf(stderr, "failed to set the clock: %s\n", strerror(errno));
			return -errno;
		}
...
void sysclk_set_leap(int leap)
{
	...
	if (clock_adjtime(clkid, &tx) < 0)
	        fprintf(stderr, "failed to set the TAI clock: %s\n", strerror(errno));
	else if (m)
		printf("%s", m);
...
void sysclk_set_sync(void)
{
	...
	if (clock_adjtime(clkid, &tx) < 0)
		                         fprintf(stderr, "failed to set the clock status: %s\n", strerror(errno));
}
